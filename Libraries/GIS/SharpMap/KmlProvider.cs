...
namespace Aricie.PortalKeeper.GIS.SharpMap
{
    /// <summary>
    /// Kml/Kmz provider, originally imported from sharpmap and modified for the needs of Aricie
    /// </summary>
    public class KmlProvider : IProvider
    {
        ...
        private const string InlineStylePrefix = "InlineForId";
        ...
        private void ExtractStyles(Element kml)
        {
            ...
            foreach (var style in kml.Flatten().OfType<Style>())
            {
                ...
                {
                    var parentPlacemark = style.GetParent<Placemark>();
                    if (parentPlacemark != null)
                    {
                        style.Id = InlineStylePrefix + parentPlacemark.Id;
                    }
                }
                else
                {
        ...
        public VectorStyle GetKmlStyle(FeatureDataRow row)
        {
            ...
            VectorStyle toReturn = null;
            // look for inline styles
            
            var rowId = (string) row["Id"];
            if (! rowId.IsNullOrEmpty())
            {
                var inlineStyleId = InlineStylePrefix + rowId;
                if (_kmlStyles.TryGetValue(inlineStyleId, out toReturn))
                {
                    return toReturn;
                }
            }
            ...
            if (_kmlStyles.TryGetValue(styleId, out toReturn))
            {
                return toReturn;
            }
        ...
        private static void AddColumnsToFeatureDataTable(FeatureDataTable fdt)
        {
            ...
            if (!fdt.Columns.Contains("Description"))
                fdt.Columns.Add("Description", typeof(string));
